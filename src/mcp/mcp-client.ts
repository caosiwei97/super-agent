...
/** 基于官方 SDK 实现的托管可流式传输 HTTP MCP 客户端。 */
export class MCPClient {
  ...
  async listTools() {
    ...
    // SDK 返回的工具结构与本模块原先约定的 MCPTool 一致，做一次显式映射便于阅读。
    return (tools || []).map((t) => ({
      name: t.name,
  ...
  async callTool(name: string, args: Record<string, unknown>) {
    if (!this.client) throw new Error('MCP client 未连接')
    // SDK 返回类型里的 `content` 是更严格的联合类型，这里统一按文本块抽取，只需做一次结构断言。
